...
#Plot Information 
x_vals = [];    packet = [];    rowContent_matrix = []
...
def killbutton():
    plt.savefig('CSV_DATA/' + filename + '_PLOTS.png')
    global switch 
    switch = False 
    rowHeader = ['TimeStamp','Roll','Pitch','Yaw','LoadX','LoadY','LoadZ']
    rowContent = [ ]
    
    with open('CSV_DATA/'+ filename  + '.csv','w',newline = '') as csvfile:
        writer = csv.writer(csvfile)  
        writer.writerow(rowHeader)
        writer.writerows(rowContent_matrix)
...
filename = 'StopRotor_Test_' + current_month + '.' + current_day + '.' + current_hour + '.' + current_minute + '.' + current_second
...
if __name__ == "__main__": 
    ...
    def animate(i):
        ...
        if switch == True:
            ...
            rowContent_matrix.append(rowContent)
